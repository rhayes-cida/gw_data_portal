		...
		for (var i = 0; i < services.length; i++) {
			...
			for (var j = 0; j < layers.length; j++) {
				...
				if (!this.getMapLayer(id)) {
					...
					} else if (serviceType == 'nmtile') {
						layerObj = new JMap.web.mapLayer.NationalMapTileLayer(layerObjParams);
					} else if (serviceType == 'nmwms') {
						layerObj = new JMap.web.mapLayer.NationalMapWMSLayer(layerObjParams);
					} else {
...
JMap.web.LayerManager.prototype.isLayerAvailable = function(layer) {
	...
	if (layer.minZoom != undefined && layer.maxZoom != undefined) {
		if (layer.minZoom <= this.map.zoom && layer.maxZoom >= this.map.zoom) {
			return true;
		}
		return false;
	}
