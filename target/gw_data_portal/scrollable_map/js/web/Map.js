...
/**
 * Removes map layer from your framework and makes it no longer available to the user.
 * 
 * @param {var} layerId	id of the layer.
 */
JMap.web.Map.prototype.unloadMapLayer = function(layerId) {
	this.layerManager.loadMapLayer(layer);
}
...
/**
 * Loads a layer into the map framework with out rendering it.  To render, you would then call appendLayer passing the ID.
 * 
 * @param {var} map layer.
 */
JMap.web.Map.prototype.loadMapLayer = function(layer) {
	layer.map = this;
	this.layerManager.loadMapLayer(layer);
}
...
/**
 * Calculates the bounding box, in pixels, of the entire map, including what is clipped
 * off by the viewport.
 * @return an object representing a bounding box xmin, ymin, xmax, ymax
 */
JMap.web.Map.prototype.getUnclippedBoundingBoxPixels = function() {
	var mapLayerContainerTop = parseInt(this.mapLayerContainer.style.top,10);
	...
	return {xmin: x, ymin: y, xmax: x + (this.tileSize * this.numTilesX), ymax: y + (this.tileSize * this.numTilesY)};
}
