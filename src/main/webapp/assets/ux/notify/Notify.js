...
Ext.EventManager.on(window, 'resize', function(){
	...
	for (var i = 0; i < Ext.ux.NotifyMgr.windows.length; ++i) {
		...
		switch (Ext.ux.NotifyMgr.alignment) {
			case 'top-center':
				Ext.ux.NotifyMgr.windows[i].el.setX(bw - parseInt(bw / 2, 10) - parseInt(w / 2, 10));
				break;
			case 'top-right':
...
Ext.ux.Notify = Ext.extend(Ext.Window, {
	...
	initComponent: function(){
		Ext.apply(this, {
			...
			msgIconCls: this.msgIconCls == null ? 'msgicon-info' : this.msgIconCls,
	...
	animShow: function(){
		...
		switch (Ext.ux.NotifyMgr.alignment) {
			case 'top-left':
				...
				break;
			case 'top-center':
				x = bw - parseInt(bw / 2, 10) - parseInt(w / 2, 10);
				y = -h;
				shift = 0;
				break;
			case 'top-right':
...
/**
 * Convenience method to show a new notification
 * box with default settings.
 * 
 * @param {String} msg The message to display.
 */
Ext.ux.Notify.notify = function(msg) {
	new Ext.ux.Notify({
		msg: msg
	}).show(document);
};
