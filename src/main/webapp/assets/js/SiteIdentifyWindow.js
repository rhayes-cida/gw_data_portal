...
var WATER_LEVEL_TAB = {
	...
	initGraph: function() {
		...
		var ISO8601TimeZoneRegex = /Z|z|[+-]\d\d:?\d\d$/;
		var ISO8601DateTimeRegex = /^(\d{4})-?(\d\d)-?(\d\d)(T(\d\d)(:?(\d\d):?(\d\d)?)?)?/;
		
		var parseISO8601Date = function(dateStr, asLocal){
