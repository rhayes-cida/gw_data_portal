...
public class HTMLUtil {
	...
	public static void getQWWellTypeList(JspWriter out, Connection connection) {
		String sql = "SELECT UNIQUE QW_WELL_TYPE_US_FLAG Value, QW_WELL_TYPE_US_FLAG Display FROM NWIS_DWH_STAR.WELL_REGISTRY WHERE qw_well_type <> 'None' ORDER BY 1";
		runQuery(sql, out, connection);
	}
	
	public static void getWLWellTypeList(JspWriter out, Connection connection) {
		String sql = "SELECT UNIQUE WL_WELL_TYPE_US_FLAG Value, WL_WELL_TYPE_US_FLAG Display FROM NWIS_DWH_STAR.WELL_REGISTRY WHERE wl_well_type <> 'None' ORDER BY 1";
		runQuery(sql, out, connection);
	}
